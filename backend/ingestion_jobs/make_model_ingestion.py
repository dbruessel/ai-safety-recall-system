...
def log_event(message: str, **fields):
    safe_fields = {k: str(v) for k, v in fields.items()}
    try:
        logger.info(message, extra={"fields": safe_fields})
    except Exception as e:
        print(f"Logging error: {e} | message={message} | fields={safe_fields}")
...
def load_checkpoint():
    ...
    if doc.exists:
        data = doc.to_dict()
        log_event("Loaded checkpoint", event="checkpoint_loaded",
                  last_make=data.get("last_make"),
                  last_model=data.get("last_model"),
                  last_year=data.get("last_year"))
        return data.get("last_make"), data.get("last_model"), data.get("last_year")
...
def save_checkpoint(make, model, year):
    ...
    log_event("Checkpoint saved", event="checkpoint_saved",
...
def get_models_for_make(make):
    ...
    log_event("Fetched models", event="fetch_models", make=make, count=len(models))
    return models

def fetch_and_store_recalls(make, model, year):
    safe_make = sanitize_make(make)
    safe_model = sanitize_model(model)
    url = f"https://api.nhtsa.gov/recalls/recallsByVehicle?make={safe_make}&model={safe_model}&modelYear={year}"
    ...
    log_event("Stored recalls", event="store_recalls",
              make=make, model=model, year=year, count=len(recalls))
    ...
    for make in makes:
        ...
        for model in models:
            ...
            for year in range(2000, 2025):
                if last_make == make and last_model == model and last_year and year <= last_year:
                    continue

                start_time = time.time()
                log_event("Starting recall fetch", event="start_fetch",
                          make=make, model=model, year=year)

                try:
                    fetch_and_store_recalls(make, model, year)
                    log_event("Recall fetch complete", event="fetch_complete",
                              make=make, model=model, year=year,
                              duration_ms=int((time.time() - start_time) * 1000))

                except requests.exceptions.HTTPError as e:
                    log_event("Skipped invalid make/model", event="skip_invalid",
                              make=make, model=model, year=year, error=str(e))

                except Exception as e:
                    log_event("Recall fetch failed", event="fetch_failed",
                              make=make, model=model, year=year, error=str(e))

                finally:
                    save_checkpoint(make, model, year)

    log_event("Ingestion complete", event="ingestion_complete")
